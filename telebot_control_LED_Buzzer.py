...
while 1:
    ...
    payload = {"ultrasonik1": ultrasonic_value, "ultrasonik2": ultrasonic_value2, "ultrasonik3": ultrasonic_value3}
    client.publish("/v1.6/devices/" + DEVICE_LABEL, json.dumps(payload), qos=1)
    time.sleep(1)
    if ultrasonic_value <8 :
        metadata = get_camera()
        counter_sensor = 0
        print(metadata)
        chat_id = 6393037362
